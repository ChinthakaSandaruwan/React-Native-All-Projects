...
router.post("/update", upload.single("image"), async (req, res) => {
  ...
  try {
    let query = "UPDATE user SET fname = ?, lname = ?, password = ? WHERE mobile = ?";
    let params = [fname, lname, password, mobile];
    let imagepath: string | null = null;

    if (req.file) {
      imagepath = "/uploads/" + req.file.filename;
      query = "UPDATE user SET fname = ?, lname = ?, password = ?, profile_picture = ? WHERE mobile = ?";
      params = [fname, lname, password, imagepath, mobile];
    }
